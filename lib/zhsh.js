...
function treedist(TD, list1, list2, i, j, tree1, tree2) {
  ...
  for (let i1 = list1[i - 1]; i1 <= i; i1++) {
    const prevCL = forestDist[i1 - 1][0] || zh.newChangeList()
    forestDist[i1][0] = {
  ...
  for (let j1 = list2[j - 1]; j1 <= j; j1++) {
    const prevCL = forestDist[0][j1 - 1] || zh.newChangeList()
    forestDist[0][j1] = {
  ...
  for (let i1 = list1[i - 1]; i1 <= i; i1++) {
    for (let j1 = list2[j - 1]; j1 <= j; j1++) {
      ...
      if (list1[i1 - 1] === list1[i - 1] && list2[j1 - 1] === list2[j - 1]) {
        ...
        const act = (base === null || target === null || base.nodeType !== target.nodeType || base.tagName !== target.tagName) ?
          rep : mod
        let modifications = []
        if (act === mod) {
        ...
        if (!TD[i1][j1]) TD[i1][j1] = zh.newChangeList()
        const delDistance = forestDist[iTemp][j1].dis + del.v
        const insDistance = forestDist[i1][jTemp].dis + ins.v
