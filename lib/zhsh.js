...
function diff(target, base) {
  ...
  console.log(`base length: ${base.map.length}`)
  console.log(`target length: ${target.map.length}`)
  ...
  for (let iBase = 1; iBase < baseKeyroots.length + 1; iBase++) {
    ...
    // console.log(`${iBase} / ${baseKeyroots.length} done`)
    for (let iTarg = 1; iTarg < targKeyroots.length + 1; iTarg++) {
      let j = targKeyroots[iTarg - 1]
...
function applyChange(base, change) {
  ...
  if (action === 'deleteNode') {
    ...
    while (baseNode.firstChild) {
      ...
      baseNode.parentNode.insertBefore(baseNode.firstChild, baseNode)
    }
