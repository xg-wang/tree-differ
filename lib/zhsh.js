...
function applyChange(base, change) {
  ...
  if (action === 'deleteNode') {
    ...
    if (newNode && baseNode.childNodes.length > 1) {
    ...
    const fc = change.firstChildIndex === null
        ? null : base.map[change.firstChildIndex - 1]
    const ns = change.nextSiblingIndex === null
        ? null : base.map[change.nextSiblingIndex - 1]
    const pt = change.parentIndex === null
        ? base.parentNode : base.map[change.parentIndex - 1]
    if (fc === null) {
