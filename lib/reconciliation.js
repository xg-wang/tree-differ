...
function diff(target, base) {
    ...
    const baseHasChildren = base.hasChildNodes()
        , targetHasChilgren = target.hasChildNodes()
    // end here if both no children
    if (!baseHasChildren && !targetHasChilgren) {
        return changes
    }
...
function apply(changes) {
    for (change of changes) {
        ...
        if (action === 'setAttribute') {
            ...
            var nodeAfter = change['parentElement'].childNodes[change['toIndex']]
            change['parentElement'].insertBefore(change['element'], nodeAfter)
        }
        // else if (action === 'moveChildElement') {
        //     change['parentElement'].insertBefore(change['element'], change['parentNode'].childNodes[change['toIndex']])
        // }
    }
}
 ...
 */
function anotherApply(change) {
    if (!change) {
        throw 'Input change is null!'
    } else if (Array.isArray(change)) {
        change.forEach(anotherApply)
    }
