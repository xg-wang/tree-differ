...
function apply(base, change) {
    ...
    if (action === 'replaceNode') {
        ...
        change.baseNode = handleChildNodes(change.baseNode, change.childChanges)
    }
    return base
}
/**
...
function handleChildNodes(parent, change) {
    if (!change || !parent) {
        ...
        for (const c of change) {
            parent = handleChildNodes(parent, c)
        }
    }
    ...
    const action = change.action
     if (action === 'removeChildElement') {
        ...
        // handle recursion
        change.element = apply(change.element, change.recursiveChanges)
    }
    return parent
}
